...
```bash
cd /path/to/Handbook
./src/rag/start_qdrant.sh
```

Wait for: `Qdrant started → http://127.0.0.1:6333`

**Or start both Qdrant and Ollama in one terminal:**
```bash
cd /path/to/Handbook
./src/rag/start_all.sh
```

**Terminal 2: Start Ollama (LLM Server)**
```bash
cd /path/to/Handbook
./src/rag/start_ollama.sh
```

Wait for: `Ollama started → http://127.0.0.1:11434`

**Note**: If you used `start_all.sh` in Terminal 1, skip this step.

**Terminal 3: Start API Server (Backend)**
```bash
cd /path/to/Handbook
conda activate SIG
...
**Optional Terminal 4: Start Frontend (Static File Server)**
```bash
cd /path/to/Handbook
python3 -m http.server 8080
```

**Note**: This is **only a static file server** for the HTML/CSS/JS files. It does **NOT** run the backend API. You must have the API server running in Terminal 3 for the chatbot to work.

Then open `http://localhost:8080/test_chatbot.html` in your browser.

**Why two terminals?** Both are long-running processes:
- Terminal 3: Backend API (handles chatbot logic, connects to Qdrant/Ollama)
- Terminal 4: Frontend server (just serves HTML files to your browser)
...
- `MANUAL_LAUNCH.md`: Detailed guide for manually launching services in separate terminals
- `SHARING_GUIDE.md`: Guide for sharing the project with colleagues (folder naming, portability, etc.)
